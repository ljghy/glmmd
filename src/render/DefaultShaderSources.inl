...
float bias;
float shadowMapCompareBilinear(vec2 uv, float currentDepth) {
    vec2 texSize = textureSize(u_shadowMap, 0);
    vec2 texelSize = 1.0 / texSize;
    vec2 texel = uv * texSize - 0.5;
    vec2 frac = fract(texel);
    vec2 base = texel - frac + 0.5;
    float tl = currentDepth - bias > texture(u_shadowMap, base * texelSize).r ? 1.0 : 0.0;
    float tr = currentDepth - bias > texture(u_shadowMap, (base + vec2(1, 0)) * texelSize).r ? 1.0 : 0.0;
    float bl = currentDepth - bias > texture(u_shadowMap, (base + vec2(0, 1)) * texelSize).r ? 1.0 : 0.0;
    float br = currentDepth - bias > texture(u_shadowMap, (base + vec2(1, 1)) * texelSize).r ? 1.0 : 0.0;
    return mix(mix(tl, tr, frac.x), mix(bl, br, frac.x), frac.y);
}
float shadow(vec4 fragPosLightSpace) {
    ...
    for (int x = -ker; x <= ker; ++x)
        for (int y = -ker; y <= ker; ++y) {
            shadow += shadowMapCompareBilinear(projCoords.xy + vec2(x, y) * texelSize, currentDepth);
        }
...
void main() {
    ...
    bias = max(0.001 * (1.0 + dot(norm, lightDir)), 0.0001);
